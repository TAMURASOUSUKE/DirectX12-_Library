...
{
	Vector3 min01{ _cube01.minPosition }; // 一つめの最小
	Vector3 max01{ _cube01.maxPosition }; // 一つめの最大
	Vector3 min02{ _cube02.minPosition }; // 二つめの最小
	Vector3 max02{ _cube02.maxPosition }; // 二つ目の最大
	// 判定を行う どれか一つでも重なっていなければ当たっていない
	if (max01.x < min02.x || min01.x > max02.x) return false;
	if (max01.y < min02.y || min01.y > max02.y) return false;
